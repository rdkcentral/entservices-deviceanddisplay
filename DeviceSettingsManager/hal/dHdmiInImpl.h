...
private:
    ...
    {
        if (bundle.OnHDMIInEventHotPlug) {
            g_HdmiInHotPlugCallback = bundle.OnHDMIInEventHotPlug;
            dsHdmiInRegisterConnectCB(DS_OnHDMIInEventHotPlug);
        }
    ...
    if (g_HdmiInHotPlugCallback) {
        ...
        LOGINFO("DS_OnHDMIInEventHotPlug event Received: port=%d, isConnected=%s", port, isConnected ? "true" : "false");
    }
