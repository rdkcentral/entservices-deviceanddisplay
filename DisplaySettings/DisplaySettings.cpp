...
namespace WPEFramework
{
    ...
    namespace Plugin
    {
        ...
        DisplaySettings::DisplaySettings()
            : PluginHost::JSONRPC(), _pwrMgrNotification(*this), _registeredEventHandlers(false), _registeredDsEventHandlers(false)
        {
            LOGINFO("constructor");
...
// Get value from ds srv persistence
        ...
        { // sample servicemanager response:{"standards":["none"],"supportsHDR":false}
            ...
            if (capabilities & dsHDRSTANDARD_HDR10)
                hdrCapabilities.Add("HDR10");
            if (capabilities & dsHDRSTANDARD_HDR10PLUS)
                hdrCapabilities.Add("HDR10PLUS");
            if (capabilities & dsHDRSTANDARD_HLG)
                hdrCapabilities.Add("HLG");
            if (capabilities & dsHDRSTANDARD_DolbyVision)
                hdrCapabilities.Add("Dolby Vision");
            if (capabilities & dsHDRSTANDARD_TechnicolorPrime)
                hdrCapabilities.Add("Technicolor Prime");
            if (capabilities & dsHDRSTANDARD_SDR)
        ...
        { // sample servicemanager response:{"success":true,"capabilities":["8 Bit","10 Bit","12 Bit","Auto"]}
            ...
            try
            {
                ...
                if (capabilities & dsDISPLAY_COLORDEPTH_8BIT)
                    colorDepthCapabilities.emplace_back("8 Bit");
                if (capabilities & dsDISPLAY_COLORDEPTH_10BIT)
                    colorDepthCapabilities.emplace_back("10 Bit");
                if (capabilities & dsDISPLAY_COLORDEPTH_12BIT)
                    colorDepthCapabilities.emplace_back("12 Bit");
                if (capabilities & dsDISPLAY_COLORDEPTH_AUTO)
            ...
            while (!_instance->m_sendMsgThreadExit)
            {
                ...
                msgInfo.param = NULL;
                {
                    ...
                    _instance->m_sendMsgCV.wait(lock, []
                                                { return (_instance->m_sendMsgThreadRun == true); });
                }
                ...
                if (currentSADState == AUDIO_DEVICE_SAD_REQUESTED)
                {
                    try
                    {
                        ...
                        if (m_currentArcRoutingState == ARC_STATE_ARC_INITIATED)
                        {
                            ...
                            if (wasSADTimerActive == true && m_arcEarcAudioEnabled == false)
                            { /*setEnableAudioPort is called, Timer has started, got SAD before Timer Expiry*/
                            ...
                            else if (m_arcEarcAudioEnabled == true)
                            { /*setEnableAudioPort is called,Timer started and Expired, arc is routed -- or for both wasSADTimerActive == true/false*/
                            ...
                            { // SAD received before setEnableAudioPort
