...
uint32_t PowerController::SetPowerState(const int keyCode, const PowerState powerState, const std::string& reason)
{
    ...
    if (PowerState::POWER_STATE_STANDBY_DEEP_SLEEP == curState
        && PowerState::POWER_STATE_STANDBY_DEEP_SLEEP != powerState
        && !_deepSleep.IsDeepSleepInProgress()) {

        // deep sleep not in progress, wakeup from deep sleep
        _deepSleep.Deactivate();
    }
