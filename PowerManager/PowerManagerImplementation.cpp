...
namespace Plugin {
    ...
    Core::hresult PowerManagerImplementation::GetTemperatureThresholds(float& high, float& critical) const
    {
        ...
        LOGINFO("Got current temperature thresholds: high: %f, critical: %f, errorCode: %u", high, critical, errorCode);
    ...
    void PowerManagerImplementation::powerModePreChangeCompletionHandler(const int keyCode, PowerState currentState, PowerState powerState, const std::string& reason)
    {
        LOGINFO("PowerModePreChangeCompletionHandler triggered keyCode: %d, powerState: %s", keyCode, util::str(powerState));

        setDevicePowerState(keyCode, currentState, powerState, reason);
