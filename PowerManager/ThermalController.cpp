...
void ThermalController::_PollThermalLevels()
{
    ...
    LOGINFO("Enter - Start monitoring temeperature every %d seconds log interval: %d, stopThread: %u", thermal_poll_interval, thermalLogInterval,_stopThread);
...
bool ThermalController::updateRFCStatus()
{
    ...
    WDMP_STATUS status = getRFCParameter(const_cast<char*>(THERMAL_PROTECTION_GROUP), "RFC_ENABLE_ThermalProtection", &param);
...
char* ThermalController::read_ConfigProperty(const char* key)
{
    ...
    WDMP_STATUS status = getRFCParameter(const_cast<char*>(THERMAL_PROTECTION_GROUP), key, &param);
