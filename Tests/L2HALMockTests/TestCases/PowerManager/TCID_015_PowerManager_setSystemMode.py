...
# Testcase ID : TCID_015_PowerManager_setSystemMode
# Testcase Description : Sets the mode of the set-top box for a specific duration before returning to normal mode. Valid modes are:
#
...
curl_response = Utils.send_curl_command(PowerManagerApis.setSystemMode)
if curl_response:
     Utils.info_log("curl command to setSystemMode is sent from the test runner")
else:
...
if str(curl_response) == str(expected_output_response):
    status = 'Pass'
    message = 'Output response is matching with expected one. setSystemMode  ' \
...
tc_id = 'TCID_015_PowerManager_setSystemMode'
