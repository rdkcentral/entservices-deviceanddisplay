...
protected:
    ...
    string response;
    IARM_EventHandler_t FrameRatePreChange = nullptr;
    IARM_EventHandler_t FrameRatePostChange = nullptr;
...
FrameRate_L2test::~FrameRate_L2test()
{
    TEST_LOG("Inside FrameRate_L2test destructor");
    uint32_t status = Core::ERROR_GENERAL;
    m_event_signalled = FrameRate_StateInvalid;
    ...
    EXPECT_EQ(Core::ERROR_NONE, status);
    TEST_LOG("Inside FrameRate_L2test destructor status value : %d",status);
}
...
TEST_F(FrameRate_L2test, setCollectionFrequencyUsingComrpc)
{
    ...
    TEST_LOG("status value : %d :: success value: %d",status,success); EXPECT_TRUE(success);
} 
...
TEST_F(FrameRate_L2test, StartFpsCollectionUsingComrpc)
{
    ...
    TEST_LOG("status value : %d :: success value: %d",status,success); 
    EXPECT_TRUE(success);
}
...
TEST_F(FrameRate_L2test, UpdateFpsUsingComrpc)
{
    ...
    if (status != Core::ERROR_NONE)
    {
        std::string errorMsg = "COM-RPC returned error " + std::to_string(status) + " (" + std::string(Core::ErrorToString(status)) + ")";
        TEST_LOG("Err: %s", errorMsg.c_str());
    }  TEST_LOG("statusSSS value : %d :: success value: %d",status,success); 
    EXPECT_TRUE(success);
}
